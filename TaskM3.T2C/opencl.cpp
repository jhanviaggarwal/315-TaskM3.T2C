...
#define DIM 100 //defining matrix size
...
void sort_array(int matrix[DIM][DIM]) {
    int temp[DIM * DIM];
    int index = 0;
    // Copy elements to a temporary 1D array
    for (int i = 0; i < DIM; ++i) {
        for (int j = 0; j < DIM; ++j) {
...
int main(int argc, char *argv[]) {
    ...
    if (val.proc_rank == 0) {
        ...
        if (DIM <= 10) {
            ...
            output(matrix2);
            printf("Result: \n");
            // Sort the result array before printing
            sort_array(res);
            output(res);
        }
        free_memory();
...
void head(int proc, int start, int end){
    ...
    setup_openCL_device_context_queue_kernel( (char*) "./ops.cl" , (char*) "multiply_matrices");
